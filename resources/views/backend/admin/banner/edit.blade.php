				...
				<div class="card-body">
					<form action="{{ route('banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<textarea name="banner_content" id="banner_content" class="form-control">{{ $banner->banner_content }}</textarea>
						...
						<div class="form-group">
							...
							<input type="text" name="banner_btn" class="form-control" value="{{ $banner->banner_btn }}">
						...
						<div class="form-group">
							...
							<input type="file" name="image" id="image" data-default-file="{{ url('upload/images', $banner->image) }}" class="form-control">
							@if($errors->has('image'))
							<span style="color: white; padding: .1rem .3rem; border-radius: 5px; background-color:crimson;">{{ $errors->first('image') }}</span>
							@endif
						</div>
						<div class="form-group">
							<button type="submit" class="btn btn-success" style="color: white;">Added</button>
						</div>
					</form>
				</div>
			</div>
